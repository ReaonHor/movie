    ...
    <van-list v-model="loading" :finished="finished" finished-text="我是有底线的" @load="onLoad" :immediate-check="false">
      <van-cell v-for="data in datalist" :key="data.filmId" @click="handledetail(data.filmId)">
        ...
        <div class="detail">
          ...
          <div><span class="labal">观众评分</span> <span class="grade">{{data.grade}}</span></div>
          <div class="actors"><span class="labal">主演：{{data.actors | actorsFilter}}</span></div>
          <div><span class="labal">{{data.nation}} | {{data.runtime}}分钟</span></div>
        </div>
        <div class="bug">购票</div>
      </van-cell>
    </van-list>
  </div>
</template>
...
export default {
  mounted () {
    http.http({
      url: `gateway?cityId=${this.$store.state.cityId}&pageNum=${this.current}&pageSize=10&type=1&k=3098000`,
